...
export default function CalculatorItemsAdmin() {
    ...
    const startAddNew = () => {
        setIsEditingId('new');
        if (activeTab === 'inverters') setEditForm({ kva: 0, voltage: 12, type: 'normal', price: 0 });
        if (activeTab === 'batteries') setEditForm({ sku: '', type: 'lithium', voltage: 12, nominal_voltage: 12, ah: 0, price: 0 });
        if (activeTab === 'panels') setEditForm({ watt: 0, price: 0 });
        if (activeTab === 'appliances') setEditForm({ name: '', running_watts: 0, surge_factor: 1 });
        if (activeTab === 'accessories') setEditForm({ kva: 0, fee: 0 });
    };
    ...
    const saveEdit = async (table: string) => {
        try {
            ...
            if (isEditingId === 'new') {
                const { error } = await supabase.from(table).insert([updateData]);
                if (error) throw error;
            } else {
    ...
    const renderHeaderAction = () => {
        if (isEditingId !== 'new') {
            return (
                <button 
                    onClick={startAddNew} 
                    className="mb-4 bg-blue-600 text-white px-4 py-2 rounded flex items-center gap-2 hover:bg-blue-700 transition"
                >
                            ...
                            <tbody className="divide-y">
                                {isEditingId === 'new' && (
                                    <tr className="bg-green-50 border-b-2 border-green-200">
                                        <td className="px-4 py-2"><input type="number" step="0.5" className="w-full p-1 border rounded" value={editForm.kva} onChange={e => setEditForm({...editForm, kva: e.target.value})} placeholder="kVA" /></td>
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.voltage} onChange={e => setEditForm({...editForm, voltage: e.target.value})} placeholder="V" /></td>
                                        ...
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.price} onChange={e => setEditForm({...editForm, price: e.target.value})} placeholder="Price" /></td>
                                        <td className="px-4 py-2 text-right whitespace-nowrap">
                                            <button onClick={() => saveEdit('calc_inverters')} className="bg-green-600 text-white px-2 py-1 rounded text-xs mr-2 hover:bg-green-700">Save</button>
                                            <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-2 py-1 rounded text-xs hover:bg-gray-300">Cancel</button>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="overflow-x-auto bg-white rounded-lg border">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y">
                                {isEditingId === 'new' && (
                                    <tr className="bg-green-50 border-b-2 border-green-200">
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.watt} onChange={e => setEditForm({...editForm, watt: e.target.value})} placeholder="Watts" /></td>
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.price} onChange={e => setEditForm({...editForm, price: e.target.value})} placeholder="Price" /></td>
                                        <td className="px-4 py-2 text-right whitespace-nowrap">
                                            <button onClick={() => saveEdit('calc_panels')} className="bg-green-600 text-white px-2 py-1 rounded text-xs mr-2 hover:bg-green-700">Save</button>
                                            <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-2 py-1 rounded text-xs hover:bg-gray-300">Cancel</button>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="overflow-x-auto bg-white rounded-lg border">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y">
                                {isEditingId === 'new' && (
                                    <tr className="bg-green-50 border-b-2 border-green-200">
                                        <td className="px-2 py-2"><input type="text" className="w-full p-1 border rounded" value={editForm.sku} onChange={e => setEditForm({...editForm, sku: e.target.value})} placeholder="SKU" /></td>
                                        ...
                                        <td className="px-2 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.ah} onChange={e => setEditForm({...editForm, ah: e.target.value})} placeholder="Ah" /></td>
                                        <td className="px-2 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.price} onChange={e => setEditForm({...editForm, price: e.target.value})} placeholder="Price" /></td>
                                        <td className="px-2 py-2 text-right whitespace-nowrap">
                                            <button onClick={() => saveEdit('calc_batteries')} className="bg-green-600 text-white px-2 py-1 rounded text-xs flex-col xl:inline-block xl:mr-2 hover:bg-green-700 mb-1 xl:mb-0">Save</button>
                                            <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-2 py-1 rounded text-xs flex-col xl:inline-block hover:bg-gray-300">Cancel</button>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="overflow-x-auto bg-white rounded-lg border">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y">
                                {isEditingId === 'new' && (
                                    <tr className="bg-green-50 border-b-2 border-green-200">
                                        <td className="px-4 py-2"><input type="text" className="w-full p-1 border rounded" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} placeholder="Appliance Name" /></td>
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.running_watts} onChange={e => setEditForm({...editForm, running_watts: e.target.value})} placeholder="Watts" /></td>
                                        <td className="px-4 py-2"><input type="number" step="0.1" className="w-full p-1 border rounded" value={editForm.surge_factor} onChange={e => setEditForm({...editForm, surge_factor: e.target.value})} placeholder="Factor" /></td>
                                        <td className="px-4 py-2 text-right whitespace-nowrap">
                                            <button onClick={() => saveEdit('calc_appliances')} className="bg-green-600 text-white px-2 py-1 rounded text-xs mr-2 hover:bg-green-700">Save</button>
                                            <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-2 py-1 rounded text-xs hover:bg-gray-300">Cancel</button>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="overflow-x-auto bg-white rounded-lg border">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y">
                                {isEditingId === 'new' && (
                                    <tr className="bg-green-50 border-b-2 border-green-200">
                                        <td className="px-4 py-2"><input type="number" step="0.5" className="w-full p-1 border rounded" value={editForm.kva} onChange={e => setEditForm({...editForm, kva: e.target.value})} placeholder="kVA Class" /></td>
                                        <td className="px-4 py-2"><input type="number" className="w-full p-1 border rounded" value={editForm.fee} onChange={e => setEditForm({...editForm, fee: e.target.value})} placeholder="Fee" /></td>
                                        <td className="px-4 py-2 text-right whitespace-nowrap">
                                            <button onClick={() => saveEdit('calc_accessories')} className="bg-green-600 text-white px-2 py-1 rounded text-xs mr-2 hover:bg-green-700">Save</button>
                                            <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-2 py-1 rounded text-xs hover:bg-gray-300">Cancel</button>
                                        </td>
                                    </tr>
                                )}
    ...
    return (
        <main className="p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    onClick={fetchAll}
                    className="px-4 py-2 border rounded text-sm bg-white hover:bg-gray-50 flex items-center gap-2 shadow-sm"
                >
                    Refresh Data
                </button>
            </div>
            ...
            <div className="mb-4">
                <p className="text-sm text-gray-500">
                    ...
                    Adding new items or changing prices is completely safe. Check and confirm new items are displaying properly on the calculator page.
                </p>
            </div>
